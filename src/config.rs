...
pub struct Config {
    ...
    pub safe_points: BTreeMap<u32, SafePoint>,
}

#[derive(Clone, Copy)]
pub struct SafePoint {
    pub voltage: u32,
    pub perf_profile: u32,
}
        ...
        let safe_points: BTreeMap<u32, SafePoint> = if let Some(array) = config.get("safe-points") {
            ...
            for (i, t) in array.iter().enumerate() {
                ...
                let perf_profile = t
                    ...
                    .map(|value| {
                        value.as_integer().ok_or_else(|| {
                            IoError::new(
                                ErrorKind::InvalidInput,
                                format!("safe-points[{i}].perf_profile must be an integer"),
                            )
                        })
                    })
                    .transpose()?
                    .unwrap_or(3);
                let perf_profile = u32::try_from(perf_profile).map_err(|_| {
                    IoError::new(
                        ErrorKind::InvalidInput,
                        format!(
                            "safe-points[{i}].perf_profile must be between 0 and {} inclusive",
                            u32::MAX
                        ),
                    )
                })?;
                ...
                if perf_profile > 3 {
                    Err(IoError::new(
                        ErrorKind::InvalidInput,
                        format!("safe-points[{i}].perf_profile must be between 0 and 3 inclusive"),
                    ))?;
                }
            ...
            for (frequency, safe_point) in &safe_points {
                let pair = (safe_point.voltage, *frequency);
